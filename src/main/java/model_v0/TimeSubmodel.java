...
public class TimeSubmodel  extends Submodel{
	...
	public void defineVariables() {
	    ...
	    //the total of operation time for rotation r at port/canal p
	      ...
	      List<Integer> dp = IntStream.range(0, nbPorts).boxed().collect(Collectors.toList());
		  dp.add(val);
	      
	      depR = getModel().getChocoModel().intVarArray("dep_r", nbRotations,dp.stream().mapToInt(Integer::intValue).toArray());
	...
	public void defineConstraints() {
       ...
       for (int r = 0; r < nbRotations; r++) {
        ...
        for (int r = 0; r < nbRotations; r++) {
        	clauseAdder.addEquivalence (((IntVar[])model.getVariable("v_r"))[r], Operator.EQ, 0, depR[r], Operator.EQ,val);
        }
