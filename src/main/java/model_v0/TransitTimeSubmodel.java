...
public class TransitTimeSubmodel extends Submodel {
	...
	public TransitTimeSubmodel(Model model) {
		...
		tableIndexIn.add(tableIndexIn.getStarValue(),val, nbPorts * nbRotations);
	...
	public void defineConstraints() {		
		...
		for(int r=0;r<nbRotations;r++) {
			for (int k : commoditiesWithTt) {
				for (int i = 0; i <= tsmax; i++) {
    		        ...
    		        IntVar prodPortRotation = getModel().getChocoModel().intVar(-10000,1000);
		...
		for (int k : commoditiesWithTt) {
				for (int i = 0; i <= tsmax; i++) {
					getModel().getChocoModel().ifThen(
							getModel().getChocoModel().arithm(((IntVar[][]) model.getVariable("rot_ki"))[k][i],"=",val),
